...
const Card = (data) => {
    ...
    const renderIcon =(id) => {
        ...
        if (isInCart) {
            return(
                <div 
                className='absolute m-2 top-0 right-0 flex justify-center items-center bg-white text-white w-6 h-6 rounded-full p-1 hover:bg-cyan-400'
                >
                    <CheckIcon className="h-6 w-6 text-cyan-600" />
                </div>
            )
        } else {
            return(
